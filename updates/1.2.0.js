/*
  ...
  # Changes in v1.2.0
  Still a work in progress, use this only to test the update validity 
        ...
        FAE.step[FAE.index + 1].data = {
          ...
          'js_placement[]' : 'allpages',
                   content : form.content.value
                             ...
                             // local frame styler
                             '\n/* -- 13. local frame styler -- */\n// global function for getting local iframes\nfunction fae_getLocalFrames (callback) {\n  var frame = document.querySelectorAll(\'iframe[src^="/"], object[data^="/"]\'),\n      i = 0,\n      j = frame.length;\n\n  for (; i < j; i++) {\n    try {\n      callback(frame[i].contentDocument || frame[i].contentWindow.document);\n    } catch (error) {\n      window.console && console.log(error);\n    }\n  }\n\n};\n\n// waits for frames to load (such as chatbox and smilies) and then applies preferred styles to them\n$(window).load(function() {\n  fae_getLocalFrames(function (frame) {\n    $(\'head\', frame).append($(\'#fa_theme_style, #fae_light-switch-css\', document.head).clone());\n  });\n\n  $(\'#fae_custom-theme, #fa_theme_selector, #fae_light-switch input\').on(\'change\', function() {\n    fae_getLocalFrames(function (frame) {\n      var head = $(\'head\', frame);\n\n      $(\'#fa_theme_style, #fae_light-switch-css\', head).remove();\n      head.append($(\'#fa_theme_style, #fae_light-switch-css\', document.head).clone());\n    });\n  });\n});\n',
